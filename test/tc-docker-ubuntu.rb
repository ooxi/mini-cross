...
class TestUbuntuDockerContext < Test::Unit::TestCase
	...
	def execute_smoke_test(version)
		...
		Dir.mktmpdir do |dir|
			...
			output = `#{DockerCli.run_cmd image, ubuntu.run, command}`
	...
	def test_Ubuntu_1810
		self.execute_smoke_test 'ubuntu:18.10'
	end

	def test_Ubuntu_1904
		self.execute_smoke_test 'ubuntu:19.04'
	end
